...
export interface WorkflowEventData {
  // Graph 层事件
  type: 'workflow_started' | 'node_started' | 'node_finished' | 'route_decision' | 'workflow_complete' |
        // Node 层事件
        'thinking_phase' | 'tool_call_pending' | 'tool_executing' | 'tool_result' | 'llm_streaming';
  level?: 'graph' | 'node';
  data?: any;
  timestamp?: number;
}
...
export function WorkflowTimeline({ events }: { events: WorkflowEventData[] }) {
  ...
  const toolEvents = events.filter(e => 
    e.level === 'node' && 
    (e.type === 'tool_executing' || e.type === 'tool_result')
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3 pb-2">
        ...
        <span className="text-[10px] text-[var(--text-tertiary)] ml-auto">{toolEvents.length} 个</span>
      </div>
...
function WorkflowEventItem({ event }: { event: WorkflowEventData }) {
  ...
  if (event.level === 'node') {
    ...
    // 只显示工具调用的结果状态
    if (event.type === 'tool_call_pending' || event.type === 'tool_executing' || event.type === 'tool_result') {
