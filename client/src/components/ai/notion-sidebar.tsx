...
export const NotionSidebar: React.FC<NotionSidebarProps> = ({
  ...
  const renderSidebarItem = (item: SidebarItem, level = 0) => (
    ...
    >
      ...
      {item.hasChildren && <ChevronRight className="w-3 h-3 opacity-50 flex-shrink-0" />}
    </motion.div>
  );
  ...
  const renderSection = (
    ...
    return (
      <div className="mb-4">
        ...
        >
          <div className="space-y-1 mt-1">{items.map(item => renderSidebarItem(item))}</div>
        </motion.div>
      </div>
    );
  };
  ...
  return (
    <motion.div
      className="w-64 h-full bg-[var(--surface-base)] flex flex-col shadow-sm"
      initial={animated ? { x: -280, opacity: 0 } : false}
      animate={animated ? { x: 0, opacity: 1 } : false}
      transition={
        animated
          ? {
              duration: 0.5,
              delay: 0.2, // 在对话框动画开始后再滑入
              ease: [0.32, 0.72, 0, 1],
            }
          : {}
      }
    >
      ...
      <div className="p-4 pb-3">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 bg-gradient-to-br from-amber-400/20 to-orange-400/20 rounded-lg flex items-center justify-center border border-amber-400/30">
            <HappyWoodsLogoIcon size={24} />
          </div>
